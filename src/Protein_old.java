...
public class Protein_old {
    ...
    Protein_old              IdenticalLoci = null;
    char                 Validated = 'U';
    char		 NChar = 'X';
    char		 CChar = 'X';
    ...
    public Protein_old Clone() {
	Protein_old  Copy = new Protein_old();
...
//	    this.ProtFP = new Float(Parser.nextToken()).floatValue();
     ...
     */
    public int Similarity(Protein_old Other) {
...
//		new Float(this.ProtFP).toString() + Tab +
    ...
    public void AccumulateDTAsFrom(Protein_old Other) {
	...
	try {
		...
		PDBBW.write(
			...
			(Cutoffs.DisplayDeltaMass && Cutoffs.ExtraColumns ? "\t" + Protein_old.RoundTo(new Float(DTARunner.Adjusted_PPM_Offset).floatValue(),1) : "") +
			"\t" + new Integer(DTARunner.Sp).toString() +
			"\t" + new Float(DTARunner.SpScore).toString() +
			(Cutoffs.DisplayPI && Cutoffs.ExtraColumns ? "\t" + Protein_old.RoundTo(DTARunner.CalculatepI(),2) : "") +
			(Cutoffs.DisplayKD && Cutoffs.ExtraColumns ? "\t" + Protein_old.RoundTo(DTARunner.CalculateKyteDoolittle(),1) : "") +
                        (Cutoffs.DisplayBB && Cutoffs.ExtraColumns ? "\t" + Protein_old.RoundTo(DTARunner.CalculateBullBreese(),1) : "") +
                        (Cutoffs.DisplayHPLC && Cutoffs.ExtraColumns ? "\t" + Protein_old.RoundTo(DTARunner.CalculateHPLCpH34(),1) : "") +
	...
	while (Runner != null) {
	    ...
	    //changed by Howard Choi
	    	...
	    	while( (SubRunner != null) &&
                   ...
                   SubRunner.FileName.substring(0,SubRunner.FileName.indexOf(".")).equals(Runner.FileName.substring(0,Runner.FileName.indexOf("."))) ){
                        Count++;
...
/*			if (Runner.Tryptic != TempTryptic)
	...
	if (Length > 0) {
	     ...
	     * the decimal point.
	     */
	    CurrentCharge = Protein_old.ChargeAtPH(CurrentPH, CountLys,
					       CountArg, CountHis,
					       CountAsp, CountGlu,
					       CountCys, CountTyr);
		...
		CurrentCharge = Protein_old.ChargeAtPH(CurrentPH,
						   CountLys, CountArg,
						   CountHis, CountAsp,
						   CountGlu, CountCys,
						   CountTyr);
    ...
    public Protein_old CreateMegaProtein() {
	Protein_old       Mega = new Protein_old();
	DTAFile       DRunner;
	Protein_old       PRunner = this.Next;
	while (PRunner != null) {
	...
	Protein_old      ThisRunner = this;
	Protein_old      OtherRunner;
	DTAFile      ThisDTA;
	DTAFile      OtherDTA;
    ...
    public void MergeAllFrom(Protein_old Other) {
	Protein_old       ThisRunner = this;
	Protein_old       OtherRunner = Other.Next;
	Protein_old       Buffer;
    ...
    public Protein_old FindProtein(String Probe) {
	Protein_old         PRunner = this.Next;
	Protein_old         PRunner2;
	while (PRunner != null) {
	    PRunner2 = PRunner;
	    while (PRunner2 != null) {
    ...
    */
	...
	Protein_old         PRunner = this.Next;
	Protein_old         IdenticalBuffer;
	Protein_old         InsidePRunner;
	DTAFile         DRunner;
	...
	Protein_old         Runner = this.Next;
	Protein_old         SeparateStorage = new Protein_old();
	Protein_old         SSRunner;
	Protein_old         RRunner;
	Protein_old         TempProt;
	while (Runner != null) {
	...
	Protein_old         OuterRunner = this.Next;
	Protein_old         Runner;
	CoverageZone    Buffer;
	...
	while (OuterRunner != null) {
		...
		Runner.SequenceCoverage = Protein_old.RoundTo((100.0f *
							   SCAccumulator /
     ...
     */
    private Protein_old Sort(Protein_old Follower) {
	//Recursive quicksorter
	//Returns first item of sorted list (from those starting at this element)
	//Accepts first item to follow
	Protein_old		ListAbove = null;
	Protein_old		ListBelow = null;
	Protein_old		PlaceHolder;
	Protein_old		PlaceHolder2;
	...
	try {
	    ...
	    // original line :Outgoing.write("LocusID\tSequenceCount\tSpectrumCount\tCoverage\tL\tMW\tpI\tDescription\tDuplicate\tClassID\n");
		...
		while (MatchRunner != null) {
		    Outgoing.write(
				   ...
				   // modified by Diego Calzolari, 11/05/2012
				   new Float(Math.pow(10, (MatchRunner.SequenceCoverage/100))-1).toString() + tab +
				   new Float(MatchRunner.SAF/NSAF_sum).toString() + tab +
				   //---------------------------------------------------------------------------------------
				   MatchRunner.Gene);
	...
	try {
	    ...
	    Outgoing.write("\n<DTASelect:Header>\n");
	    Outgoing.write("\t<DTASelect:Version>" + Protein_old.Version() + "</DTASelect:Version>\n");
